...
enum InformantBotStepEnum: int
{
    ...
    public function getStepName(): string
    {
        return match ($this) {
            ...
            self::S14_Q => '1 Этап - 14',
            self::T1, self::T10, self::T9, self::T8, self::T7, self::T6, self::T5, self::T4, self::T3, self::T2 => 'Тестирование',
            self::S21 => 'Прошел тестирование',
            self::REVIEW => 'Отзыв',
            self::FINISH => 'Конец',
            self::START_FAIL  => 'Отзыв: В начале ответил: Не очень хочется, но придётся',
            self::START_FINISH => 'Конец. В начале ответил: Не очень хочется, но придётся',
        };
    }
...
Каждый день мы отправляем по информационно - телекоммуникационная сети десятки электронных сообщений",
            ...
            self::FINISH, self::START_FINISH => null,
...
ВАЖНО ❗️ Обработка персональных данных осуществляется с согласия субъекта персональных данных на обработку",
...
Ценность информации изменяется во времени. К изменению ценности информации приводят распространение информации и её использование.',
            ...
            self::S13_Q => 'В современном мире детям особенно важно соблюдать культуру безопасного поведения в информационной среде
      
Хочешь скачать квест-игру «Информационная безопасность и кибергигиена»?',
            self::S14_Q => 'Предлагаю пройти тестирование для оценки усвоенного материала',
            self::S21 => 'При составлении информационной интерактивной сессии использовались следующие информационные источники:
...
г) материал судебного расследования.',
            self::T6 => '6. Определите особенности сети «Интернет»
а) высокая скорость и оперативность предоставления информации;
б) широкая аудитория;
в) практически неограниченный выбор источников и видов информации;
...
в) переслать в государственное подразделение по секретному каналу.',
    ...
    public function isRightTestAnswer(string $text): bool
    {
        $rightAnswer = match ($this) {
            self::T1 => 'б) сведения, сообщения, данные независимо от формы их представления',
            self::T2 => 'а) технологическая система, предназначенная для передачи по линиям связи информации, доступ к которой осуществляется с использованием средств вычислительной техники',
            self::T3 => 'е) всё перечисленное верно',
            self::T4 => 'б) часть территории Российской Федерации, в границах которой наряду с государственным управлением осуществляется местное самоуправление',
            self::T5 => 'в) нормативно-правовой акт, затрагивающий права человека',
            self::T6,
            self::T9,
            self::T8 => 'г) всё перечисленное верно',
            self::T7 => 'б) это онлайн-платформа, которая используется для общения, знакомств, развлечения и работы',
            self::T10 => 'а) уничтожить',
            default => false,
        };
